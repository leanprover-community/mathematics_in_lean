...
theorem primes_infinite : ∀ n, ∃ p > n, Nat.Prime p := by
  ...
  simp_all only [Nat.isUnit_iff, Nat.dvd_one]
...
example : (r ∪ s) ∩ (r ∪ t) = r ∪ s ∩ t := by
  ...
  rename_i inst
  unhygienic ext
  simp_all only [mem_inter, mem_union]
  apply Iff.intro
  · intro a_1
    unhygienic with_reducible aesop_destruct_products
    unhygienic aesop_cases left <;> [(unhygienic aesop_cases right); (unhygienic aesop_cases right)]
    · simp_all only [true_or]
    · simp_all only [and_true, true_or]
    · simp_all only [true_and, true_or]
...
example : (r \ s) \ t = r \ (s ∪ t) := by
  ...
  rename_i inst
  unhygienic ext
  simp_all only [mem_sdiff, mem_union]
  apply Iff.intro
  · intro a_1
    simp_all only [or_self, not_false_eq_true, and_self]
  · intro a_1
    ...
    · apply Aesop.BuiltinRules.not_intro
      intro a_1
      simp_all only [true_or, not_true]
    · apply Aesop.BuiltinRules.not_intro
      intro a_1
      simp_all only [or_true, not_true]
...
theorem primes_infinite' : ∀ s : Finset Nat, ∃ p, Nat.Prime p ∧ p ∉ s := by
  ...
  -- aesop?
  rename_i this_1 this_2
  simp_all only [mem_filter, and_iff_right_iff_imp, implies_true, forall_const, Nat.isUnit_iff, Nat.dvd_one]
...
theorem primes_mod_4_eq_3_infinite : ∀ n, ∃ p > n, Nat.Prime p ∧ p % 4 = 3 := by
  ...
  have h₁ : ((4 * ∏ i in erase s 3, i) + 3) % 4 = 3 := by
    ...
    ring
    simp_all only [Nat.add_mul_mod_self_right]
  rcases exists_prime_factor_mod_4_eq_3 h₁ with ⟨p, pp, pdvd, p4eq⟩
  have ps : p ∈ s := by
    -- aesop?
    simp_all only [gt_iff_lt, ne_eq, Nat.isUnit_iff]
    have := hs p
    simp_all only [Nat.isUnit_iff, and_self, true_iff]
  have pne3 : p ≠ 3 := by
    sorry
